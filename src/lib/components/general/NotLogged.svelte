<script lang="ts">
	...
	function setCLassByStreaming(streaming: string) {
		let baseClass: string =
			'inline-flex items-center cursor-pointer justify-center gap-2 rounded-lg px-6 py-3.5 text-sm font-semibold text-t-inverse shadow-md hover:scale-102';

		if (streaming === 'spotify') {
			baseClass = `${baseClass} bg-spotify`;
		} else {
			baseClass = `${baseClass} bg-deezer`;
		}
...
<div
	class="flex flex-col items-center justify-center gap-6 px-6 py-12 text-center sm:px-8 sm:py-16 lg:px-12 lg:py-20"
>
