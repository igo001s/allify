	...
	>
		{#each $userInfo?.connectedStreamings.spotify?.mostListenedArtists?.mostListenedArtistsItems as artist}
			...
			>
				...
				<SpotifyIcon
					iconSvgClass={`${choosedArtist?.id === artist.id || (!choosedArtist && $userInfo?.artists?.artistOfTheMoment?.artist?.id === artist.id) ? 'text-brand-primary' : 'text-t-secondary/70'} absolute top-1.5 right-1.5 h-3.5 w-3.5 sm:top-2 sm:right-2 sm:h-4 sm:w-4`}
				/>
			</button>
		{/each}
	</div>
