	...
	async function handleSelectCustomArtist() {
		...
		if (!updateCustomArtistResponse.error) {
			...
			toastStore.set({
				...
				toastType: 'success',
				toastMessage:
					$translationsStore.profilePage.profilePageSelectCustomArtistSuccessToastMessage
			});
			...
			toastStore.set({
				showToast: true,
				toastType: 'error',
				toastMessage: $translationsStore.profilePage.profilePageSelectCustomArtistErrorToastMessage
			});
...
<div class="flex w-full flex-col gap-4">
	...
	<div class="max-h-60 overflow-y-auto pr-2 sm:max-h-96">
		<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-6 md:grid-cols-4">
			{#each $userInfo?.connectedStreamings.spotify?.mostListenedArtists?.uniqueArtists as artist, i}
				<SelectArtistCardOnProfile key={i} {artist} {choosedArtist} {handleArtistSelection} />
			{/each}
		</div>
	...
	<div class="mt-1 flex flex-col-reverse gap-2 sm:mt-4 sm:flex-row sm:justify-end sm:gap-3">
		<button class="button-secondary px-6 py-2.5" on:click={closeSelectCustomItemModal}>
