<script lang="ts">
	...
	export let publicUserName: string;
	export let musicType: string;
</script>
...
<div class="flex w-full flex-col gap-4 xl:w-1/3">
	...
	>
		...
		<div class="max-w-[16rem] text-center sm:max-w-sm">
			...
			<p class="mt-1 text-xs leading-relaxed text-t-secondary">
				{$translationsStore.musicCommunityPage.publicUser.musicCommunityPagePublicUserEmptyMusicOnPublicUserParagraph1.replace(
					'{publicUserName}',
					publicUserName
				)}
			</p>
